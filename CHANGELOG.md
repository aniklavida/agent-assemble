...
- **Public documentation and authoring guide** — README comprehensive overview
  covering what the tool is, what it explicitly is not, a verified five-minute
  start, the four-state host support matrix, and known limitations. A dedicated
  authoring guide in `docs/AUTHORING.md` documents how to author roles,
  principles, and board adapters in alignment with the repository's verification
  scripts. Gaps in contributing guidelines, code of conduct enforcement, and
  security policies are filled.
- **Dogfood run execution** — Agent Assemble dogfooded on itself to carry a full
  work item end-to-end through its setup, board, role, and log mechanisms. The
  complete execution and observed friction points are documented in
  `docs/DOGFOOD.md`.
