#!/usr/bin/env bash
# obsidian-broken.sh — bad-fixture double for scripts/check-board-adapters.sh.
#
# Identical to obsidian.sh except that Move Card moves the file but leaves the
# frontmatter `status` stale — the exact defect adapters/obsidian.md warns
# against ("never leave the frontmatter and the folder disagreeing"). The
# equivalence check must detect the stale status. If it does not, the test is
# not load-bearing.

board_create() {
  local root="$1" id="$2" title="$3"
  local dir="$root/Tasks/todo"
  mkdir -p "$dir"
  cat > "$dir/$id.md" <<CARD
---
id: "$id"
...
board_move() {
  ...
  local src dst file
  file="$(find "$root/Tasks" -name "$id.md" -print -quit 2>/dev/null || true)"
  [ -n "$file" ] || { echo "board_move: no card $id" >&2; return 1; }
  src="$file"
  dst="$root/Tasks/$status/$id.md"
  mkdir -p "$root/Tasks/$status"
  # Defect: the file moves, but `status:` is never rewritten.
  sed -e "s/^assigned_role: .*/assigned_role: \"$role\"/" \
      -e "s#^tags: .*#tags: [board/$status]#" \
