#!/usr/bin/env bash
# obsidian.sh — executable test double for board/references/adapters/obsidian.md.
#
# Test infrastructure, not shipped content. A vault is Markdown, so this shares
# the Markdown double's mechanics; only the vault paths and the frontmatter
# `tags` convention differ. Cards live under Tasks/<status>/ inside the vault.
